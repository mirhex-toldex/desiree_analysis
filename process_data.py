...
def triangle_filters(file, start_time, doppler_df): # for scans that have both freq directions 
    ...
    filtered_df = doppler_df[doppler_df[time] >= start_time] # cut off first 25 s of data for when before laser is scanning. Will need to be changed for some scans ie night scans 
...
def get_scale(file, start_time, doppler_df: pd.DataFrame) -> list:
    ...
    for cycle, group in grouped_cycle:
        ...
        # Get the fitted line and coefficients
        ...
        cycle_scale.append({'cycle': cycle, 'coefficients': coeffs.tolist()}) 
    ...
    return cycle_scale, filtered_df
...
def process_dfs(doppler_df: pd.DataFrame, scale_info: list) ->  pd.DataFrame:
    # bkg = get_bkg(doppler_df) # probably won't use 
    bkg = 5
    freq_range = (doppler_df['Laser Frequency (THz)'].max() - doppler_df['Laser Frequency (THz)'].min()) * 1e6 # MHz
    bin_width = 25 # 50 MHz 
    scaled_bins = int(np.ceil(freq_range / bin_width))
    ...
    for info_by_cycle in scale_info:
        ...
        scaled_cycle_df = (
            cycle_df
            .assign(scaled_freq=lambda df: a * df['Time (sec)']**5 + b * df['Time (sec)']**4 + c * df['Time (sec)']**3 + 
                    d * df['Time (sec)']**2 + e * df['Time (sec)'] + f)
        )
    ...
    return scaled_df, time_df, freq_df
...
def get_dfs(folder_path):
    for filename in os.listdir(folder_path):
        if filename.endswith('.tdms'):
            ...
            scaled_df, time_df, freq_df = process_dfs(filtered_df, cycle_scales)
            yield scaled_df, time_df, freq_df
